...
> **最新实现以「六、v7.0 增量需求 — 动作结算弹窗重设计」为准。** 本节为 v5.0 原始蓝图，实际已内联在 `pages/index/index.vue`（非独立 `ActionSheet.vue`），刷题录入升级为带名称/说明的量化框 + 正确率圆环，泛读步进器改为带说明的输入框。
...
## 六、 v7.0 增量需求 — 首页「动作结算」弹窗重设计

> 本章在 v6.0 基础上整合 P11 阶段的增量需求，是「单项任务结算弹窗」的当前最终蓝图。背景：原刷题结算只有两个无标注数字框，看不清要填什么。已落地于 `pages/index/index.vue` 内联弹窗（非独立 `ActionSheet.vue`），数据模型与 `settleTask` 入参零改动，通过类型检查、H5/小程序双端构建与 H5 浏览器逐功能走查。

### 6.1 触发与定位

* 由首页今日任务卡片点击唤起的**半屏底部弹窗**，强制遮罩，`.sheet` 设 `max-height: 86vh + overflow-y:auto` 适配加长内容。
* 仅负责**单项任务的客观量化结算**；完成后任务卡片移除、底部终极打卡按钮进度 +1。与「终极打卡随笔弹窗」（§二核心组件 2）是两个独立弹窗，不要混淆。

### 6.2 头部与动作分段

* 头部：`🎯` 图标 + 主标题「完成本节」+ 节点名副标题。
* 动作切换由原 `picker` 升级为**分段控件 Segment**：`刷题 / 背诵 / 学习输入`（对应 ActionType `objective / recite / comprehensive`）。选中项白底主题色高亮，其余浅灰。

### 6.3 刷题（objective）量化清晰化 —— 本次核心诉求

* **两个带名称 + 说明的量化框**，彻底解决「看不清填什么」：
  * `总刷题数`，下方灰字说明「本次共做多少题」；
  * `错题数`，红色强调（`#b91c1c`），说明「其中做错多少题」。
* **正确率圆环**：右侧 `conic-gradient` 圆环 + 中心百分比文字，随输入**实时联动**。
  * 正确率 = `(总刷题数 − 错题数) / 总刷题数 × 100%`，与 `settleObjective` 口径一致。
  * 阈值配色：`≥80%` 主题绿 `#0f766e` / `≥60%` 琥珀 `#b45309` / `<60%` 战损红 `#b91c1c`。
  * 中心百分比文字为信息主源，保证个别端环弧不渲染时信息不丢。
* **录入校验与规整**：`@blur` 时 `normalizeObjective` 取整并 clamp（`总题≥0`、`0≤错题≤总题`）；提交时若 `总题≤0` 用 `uni.showToast` 拦截。

### 6.4 背诵（recite）四挡锚点

* 由原 slider 改为**四张锚点卡片**，点选高亮：`20% 漏点 / 50% 词不达意 / 80% 踩准 / 100% 肌肉记忆`，更直观、触控友好。

### 6.5 学习输入（comprehensive）

* `耗时(分钟)`（说明「本次有效投入」）+ `踩分(0-100)`（说明「核心得分点占比」）两个带说明量化框。

### 6.6 避坑随笔 + 联动快捷标签

* `<textarea>` 避坑随笔，Placeholder「记下核心避坑口诀或感悟...」，`maxlength=200`。
* 下方**快捷标签 chips**，按当前动作类型联动：
  * 刷题：`#概念混淆 / #主体搞错 / #时间限定错误`
  * 背诵：`#关键词遗漏 / #顺序混乱 / #理解偏差`
  * 学习输入：`#踩点不全 / #时间超限 / #思路卡顿`
* 点击标签把 `#标签 ` 追加到随笔，已存在则去重。

### 6.7 智能提示栏

* 弹窗底部一条浅绿提示栏（`✨` + 文案），**纯前端按阈值计算**、嵌入当前节点名，无任何网络/大模型依赖：
  * 刷题：按正确率分档（≥90 拉长间隔 / ≥60 错题需二次订正 / <60「错误率较高，建议重点复盘【节点名】」）。
  * 背诵：按掌握度分档（100 进入长周期复习 / 80 明日快速回忆 / 50 关键词重背 / 20 重学后再背）。
  * 学习输入：按踩分分档。
* **文案取舍备注**：用户提供的效果图中此栏标注为「AI 洞察」，但本项目 §〇 与 v6.0 §5.1 已确立「全站文案去 AI 化」纪律，二者冲突。当前实现先采用中性的「智能提示」，最终以「AI 洞察 / 智能提示」何者为准由产品决策后统一。

### 6.8 主操作

* 主按钮文案：`标记完成并闭环`（沿用 §App.vue 的 button 双层 reset，单色无渐变）。
* 次按钮：`取消`，关闭弹窗。
