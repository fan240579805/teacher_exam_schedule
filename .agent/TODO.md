...
按用户效果图把首页任务点击后的结算弹窗做清晰化，仅在 P10.1 基线上叠加，不回退既有首页改动。

- [x] 头部改为 `🎯 完成本节` + 节点名 + `刷题/背诵/学习输入` 分段控件（替代 picker）。
- [x] 刷题量化框加名称（总刷题数/错题数）+ 说明文字 + 错题数红色强调，含义清晰。
- [x] 新增正确率圆环（conic-gradient 实时联动，≥80 绿 / ≥60 琥珀 / <60 红），口径与 `settleObjective` 一致。
- [x] 背诵改四挡锚点卡片（20% 漏点 / 50% 词不达意 / 80% 踩准 / 100% 肌肉记忆）。
- [x] 学习输入保留耗时/踩分两框并加说明。
- [x] 避坑随笔改 `<textarea>` + 按动作类型联动的快捷标签 chips（点击追加去重）。
- [x] 新增智能提示栏（阈值计算文案 + 节点名，无网络）。
- [x] 主按钮改 `标记完成并闭环`，提交前加刷题 `总题>0、0≤错题≤总题` 校验；数据模型与 `settleTask` 入参不变。
- [x] 修复实现中误整文件覆盖回退 P9~P10.1 的问题：恢复 HEAD 基线后改为精准叠加。
- [x] 验证：`pnpm type-check`、IDE lints、`pnpm build:h5`、`pnpm build:mp-weixin`、H5 浏览器逐功能走查均通过。
